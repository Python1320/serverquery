...
local function query_master(master_retries_remaining, cb, filter, region, host, port, nomore)
	...
	-- get a reply from master server
	while true do
...
local function rebuild_format(fmt)
	...
	for k, v in next, fmt do
		vstruct_format[k] = v[1]
	end
...
local function parse_multipart(entry, dat)
	...
	if ismultipart and hasallparts then
		PrintTable(entry)
		assert(false, "!?")
	end
...
------------- the meat ------------
local function GENERATE(__payload__, __parse__)
	local function generated(cb, max_parallel, max_wait, max_tries)
		...
		local function add_queue(ip, port)
			local entry = {ip, port, false, false, max_tries, {}}
